...
def check(getopt):
	...
	if payload > 6 and getopt.payload is None:
		...
		if answer.lower().strip() == "y":
			Log.info("Write the DXD MASA payload below")
			payload=input("> "+W)
		else:
...
def start():
	parse=argparse.ArgumentParser(formatter_class=argparse.RawTextHelpFormatter,usage="DXD ASA -u <target> [options]",epilog=epilog,add_help=False)
